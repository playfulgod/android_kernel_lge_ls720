...
void __init lge_add_input_devices(void)
{
	...
	/* F6_TMUS rev.A blocking touch_reset pin */
	rc = gpio_request(TS_GPIO_RESET, "touch_reset");
	if(rc < 0) {
	     pr_err("%s: touch_reset(52) request fail\n", __func__);
	}
	gpio_direction_output(TS_GPIO_RESET, 0); 	 
