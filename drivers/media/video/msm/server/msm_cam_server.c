...
/*                                                                                       */
			if (!(current->flags & PF_EXITING)){
			      pr_err("kill camera daemon client aka mediaserver");
				send_sig(SIGKILL, current, 0);
			}
/*                                                                                       */
			/*                                                            */

			//pr_err("%s: forcing crash by soojong.\n", __func__); 
...
/*                                                                    */
	...
	if(g_server_dev.use_count > 0)
	{
		...
		rc = wait_event_interruptible_timeout(g_server_dev.ft_wq, 
			(g_server_dev.prev_task == NULL || g_server_dev.prev_task == current->group_leader),
			g_server_dev.wait_ft_timeout);

		if(!(g_server_dev.prev_task == NULL || g_server_dev.prev_task == current->group_leader))
		{
			printk(KERN_DEBUG "%s : fail to wait for task free - count : %d, %s, pid = %d\n", __func__, 
	...
	//anyway..
	g_server_dev.prev_task = current->group_leader;
#endif
/*                                                                    */
...
/*                                                                    */
	...
	g_server_dev.wait_ft_timeout = msecs_to_jiffies(500); //500ms
#endif
/*                                                                    */
