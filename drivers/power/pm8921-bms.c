						...
						int chargecycles)
{
	...
	//pr_info("[BMS-POWER] fcc_uah = %d ocv = %d pc = %d\n", fcc_uah, ocv, pc);
						...
						bool calculate_iavg_flag)
{
	...
	*remaining_charge_uah = calculate_remaining_charge_uah(chip, raw,
	...
	if(calculate_iavg_flag){
	printk("[BMS-POWER][calculate_soc_params] cc_uah = %duAh raw->cc = %x cc = %lld after subtracting %x\n",
				*cc_uah, raw->cc,
		...
		int rbatt, int fcc_uah, int uuc_uah, int cc_uah)
{
	...
	if (ibat_ua < 0 && pm8921_is_batfet_closed()) {
		...
		pr_info("[BMS-POWER] ibat_ua < 0 && pm8921_is_batfet_closed(), so go out\n");
		goto out;
	}
   ...
   */
	...
	if (soc_new == 0 && soc_est != 0 && (vbat_uv > 3200000) ){
		soc_new = 1;
	}
	if(soc_new == 0 && (vbat_uv > 3200000) )
	{
		soc_new = 1;
	pr_info("[BMS-POWER] soc_new == 0 && (vbat_uv > 3200000), so soc = 1\n");
	}
	if(soc_new == 1 && (vbat_uv < 3200000) ) 
	{
		soc_new = 0;
	pr_info("[BMS-POWER] soc_new == 1 && (vbat_uv < 3200000), so soc = 0\n");
	}
	if(soc_est == 0 && (vbat_uv < 3200000) )
	{
		soc_new = 0;
	pr_info("[BMS-POWER] soc_est == 0 && (vbat_uv < 3200000), so soc = 0\n");
	}
	if(soc_new == 2 && (vbat_uv < 3100000) ) 
	{
		soc_new = 0;
	pr_info("[BMS-POWER] soc_new == 2 && (vbat_uv < 3100000), so soc = 1\n");
	}
#else
...
static bool is_shutdown_soc_within_limits(struct pm8921_bms_chip *chip, int soc)
{
	if (shutdown_soc_invalid) {
		pr_info("[BMS-POWER] NOT forcing shutdown soc = %d\n", chip->shutdown_soc);
		return 0;
	}
 ...
 */
	...
	if (vbatt < 3050000)
	{
		pr_info("[BMS-POWER] vbat < 3050000 !!!! \n");
		count++;
		if (count > I_MAX)
		{
			return 0;
