						...
						uint16_t gain, uint32_t line)
{
        ...
        uint16_t fl_lines_tmp;
        uint16_t ex_lines_tmp;
...
//        offset = s_ctrl->sensor_exp_gain_info->vert_offset;
        ...
        msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
                        s_ctrl->sensor_output_reg_addr->frame_length_lines,&fl_lines_tmp,
                                        MSM_CAMERA_I2C_WORD_DATA);
        msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
                        s_ctrl->sensor_exp_gain_info->coarse_int_time_addr,&ex_lines_tmp,
                                        MSM_CAMERA_I2C_WORD_DATA);

        if(line < (fl_lines -6))
                        fl_lines_tmp = fl_lines + ((0x0001 & fl_lines)?(0x0001 & ~ ((ex_lines_tmp ^ line))) : (0x0001 & ((ex_lines_tmp ^ line))));
        else
                        fl_lines_tmp = line + 8 + (0x0001 &(line+8)?(0x0001 & ~ ((ex_lines_tmp ^ line))) : (0x0001 & ((ex_lines_tmp ^ line))));

        fl_lines = fl_lines_tmp;
